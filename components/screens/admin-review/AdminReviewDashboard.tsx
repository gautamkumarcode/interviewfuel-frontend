...
export function AdminReviewDashboard() {
	...
	>([]);
	const [inReviewQuestions, setInReviewQuestions] = useState<
		GetAllQuestionsResponseType[]
	>([]);
	...
	const fetchData = async () => {
		try {
			setLoading(true);
			const [statsData, pendingData, inReviewData] = await Promise.all([
				questionService.getReviewStatistics(),
				questionService.getPendingQuestions({ reviewStatus: "pending" }),
				questionService.getPendingQuestions({ reviewStatus: "in_review" }),
			]);

			setStats(statsData.data);
			setPendingQuestions(pendingData.data.results);
			setInReviewQuestions(inReviewData.data.results);
		} catch (error) {
										...
										<div className="flex-1 space-y-2">
											...
											<div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
												...
												{question.verifiedBy && (
													<>
														<span>•</span>
														<span>Reviewed by {question.verifiedBy.name}</span>
													</>
												)}
											</div>
										</div>
